...
const SECTIONS: { key: string; title: string; icon: string }[] = [
  ...
  { key: 'display', title: 'Storefront Display', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zm10 0a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z' },
...
const FIELDS: Record<string, FieldMeta> = {
  ...
  // Orders & Payment
  payment_timeout_minutes: { label: 'Payment Timeout', type: 'number', section: 'order' },
  ...
  max_pending_orders: { label: 'Max Pending Orders Per Customer', type: 'number', section: 'order' },
...
export default function SettingsPage() {
  ...
  function displayValue(s: Setting) {
    ...
    if (meta?.type === 'boolean') return s.value === 'true' ? 'Yes' : 'No';
    return s.value || '—';
  }
  ...
  function settingsForSection(sectionKey: string) {
    ...
    <div>
      ...
      <div className="space-y-6">
        {SECTIONS.map(section => {
          const items = settingsForSection(section.key);
          if (items.length === 0) return null;
          return (
            <div key={section.key} className="bg-white border border-cream-deep/60 overflow-hidden">
              <div className="px-5 py-4 bg-cream-warm/30 border-b border-cream-deep/40 flex items-center gap-3">
                <svg className="w-5 h-5 text-bark-light/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d={section.icon} />
                </svg>
                <h2 className="font-display text-base font-semibold text-bark">{section.title}</h2>
              </div>
                      ...
                      {meta?.type === 'boolean' ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className={`input-field !py-1.5 !text-sm ${meta?.type === 'text' ? '!w-48' : '!w-28'}`}
                            autoFocus
                          />
