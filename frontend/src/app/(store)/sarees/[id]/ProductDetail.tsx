...
function Stars({ rating, size = 'w-4 h-4' }: { rating: number; size?: string }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(i => (
        <svg key={i} className={`${size} ${i <= rating ? 'text-gold fill-gold' : 'text-bark-light/20'}`} viewBox="0 0 24 24" fill={i <= rating ? 'currentColor' : 'none'} stroke="currentColor">
    ...
    <div className="flex gap-1">
      {[1,2,3,4,5].map(i => (
        <button key={i} type="button"
          ...
          className="p-0.5"
        >
          <svg className={`w-7 h-7 transition-colors ${i <= (hover || rating) ? 'text-gold fill-gold' : 'text-bark-light/20 hover:text-gold/40'}`}
            viewBox="0 0 24 24" fill={i <= (hover || rating) ? 'currentColor' : 'none'} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
          </svg>
        </button>
      ))}
...
function ReviewSection({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    if (!productId) return;
    fetch(`${API_URL}/api/products/${productId}/reviews`).then(r => r.ok ? r.json() : []).then(setReviews).catch(() => {});
    fetch(`${API_URL}/api/products/${productId}/reviews/summary`).then(r => r.ok ? r.json() : { averageRating: 0, reviewCount: 0 }).then((d: { averageRating: number; reviewCount: number }) => setSummary({ averageRating: d.averageRating || 0, reviewCount: d.reviewCount || 0 })).catch(() => {});
  }, [productId, API_URL]);

  async function submitReview() {
    if (formRating === 0) { setSubmitError('Please select a rating'); return; }
    setSubmitting(true);
    setSubmitError('');
    try {
  ...
  return (
    <section className="mt-16 border-t border-cream-deep/40 pt-12">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="font-display text-2xl font-bold text-bark">Customer Reviews</h2>
          {summary.reviewCount > 0 && (
            <div className="flex items-center gap-3 mt-2">
              <Stars rating={Math.round(summary.averageRating)} size="w-5 h-5" />
              <span className="font-ui text-sm text-bark">{summary.averageRating} out of 5</span>
              <span className="font-ui text-xs text-bark-light/50">({summary.reviewCount} review{summary.reviewCount !== 1 ? 's' : ''})</span>
            </div>
          )}
        </div>
        {isLoggedIn && !showForm && !submitted && (
          <button onClick={() => setShowForm(true)} className="btn-outline rounded-lg text-sm">Write a Review</button>
        )}
        {!isLoggedIn && (
          <p className="font-ui text-xs text-bark-light/50">Login and purchase to write a review</p>
        )}
      </div>

      {/* Submit form */}
      {showForm && (
        <div className="bg-cream-warm border border-cream-deep/60 p-6 rounded-lg mb-8">
          <h3 className="font-display text-lg font-semibold text-bark mb-4">Write Your Review</h3>
          <div className="space-y-4">
            <div>
              <label className="input-label">Rating *</label>
              <ClickableStars rating={formRating} onChange={setFormRating} />
            </div>
            <div>
              <label className="input-label">Title</label>
              <input type="text" value={formTitle} onChange={e => setFormTitle(e.target.value)} className="input-field" placeholder="Summarize your experience" maxLength={200} />
            </div>
            <div>
              <label className="input-label">Your Review</label>
              <textarea value={formComment} onChange={e => setFormComment(e.target.value)} className="input-field resize-y" rows={4} placeholder="What did you like or dislike? How was the fabric quality?" />
            </div>
            {submitError && <p className="font-ui text-xs text-red-500">{submitError}</p>}
            <div className="flex gap-3">
              <button onClick={submitReview} disabled={submitting || formRating === 0} className="btn-primary rounded-lg !py-2 text-sm disabled:opacity-50">
                {submitting ? 'Submitting...' : 'Submit Review'}
              </button>
              <button onClick={() => setShowForm(false)} className="btn-outline rounded-lg !py-2 text-sm">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {submitted && (
        <div className="bg-green-50 border border-green-200 p-4 rounded-lg mb-8 text-center">
          <p className="font-ui text-sm text-green-700">Thank you for your review! It will appear after admin approval.</p>
        </div>
      )}
      ...
      {reviews.length === 0 && !showForm && !submitted ? (
        <div className="text-center py-10">
          <p className="font-body text-bark-light">No reviews yet. Be the first to share your experience!</p>
        </div>
      ) : (
        <div className="space-y-6">
          {reviews.map(r => (
            <div key={r.id} className="border-b border-cream-deep/30 pb-6 last:border-0">
              <div className="flex items-center gap-3 mb-2">
                <Stars rating={r.rating} />
                <span className="font-ui text-sm font-medium text-bark">{r.reviewerName || 'Customer'}</span>
                {r.isVerified && (
                  <span className="font-ui text-[10px] px-1.5 py-0.5 bg-green-50 text-green-700 rounded border border-green-200">Verified Purchase</span>
                )}
...
function RelatedProducts({ productId }: { productId: string }) {
  ...
  useEffect(() => {
    if (!productId) return;
    fetch(`${API_URL}/api/products/${productId}/related?limit=4`)
      .then(r => r.ok ? r.json() : [])
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const { default: mapFn } = require('@/lib/api');
          // Map backend Product to frontend Saree
          setRelated(data.map((p: Record<string, unknown>) => ({
            id: p.id as string,
            name: p.name as string,
            description: (p.description as string) ?? '',
            priceInPaisa: (p.sellingPrice as number) ?? 0,
            mrpInPaisa: (p.mrp as number) ?? 0,
            fabric: (p.fabric as string) ?? 'SILK',
            weave: (p.weaveType as string) ?? 'IKAT',
            color: (p.bodyColor as string) ?? '',
            lengthInMeters: (p.lengthMeters as number) ?? 6.0,
            blousePieceIncluded: (p.blousePiece as boolean) ?? false,
            images: (p.images as string[]) ?? [],
            stock: (p.stock as number) ?? 0,
            active: (p.isActive as boolean) ?? true,
            gstPct: (p.gstPct as number) ?? 5,
            createdAt: (p.createdTime as string) ?? new Date().toISOString(),
          })));
        }
      })
      .catch(() => {});
  ...
  // Import SareeCard dynamically to avoid circular deps
  const SareeCard = require('@/components/SareeCard').default;

  return (
    <section className="mt-16 border-t border-cream-deep/40 pt-12">
      <div className="text-center mb-8">
        <span className="font-ui text-xs tracking-[0.2em] uppercase text-gold">You May Also Like</span>
        <h2 className="mt-2 font-display text-2xl font-bold text-bark">Similar Sarees</h2>
